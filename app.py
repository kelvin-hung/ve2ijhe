...
def _ensure_tmp_dataset_root() -> Path:
    """Create (once per session) a temp folder for uploaded datasets."""
    key = "_ve2plume_tmp_dataset_root"
    if key not in st.session_state:
        st.session_state[key] = tempfile.mkdtemp(prefix="ve2plume_dataset_")
    return Path(st.session_state[key])

def _prepare_uploaded_dataset_from_zip(uploaded_zip) -> str:
    """Extract uploaded ZIP and return the folder that contains data.mdb."""
    if uploaded_zip is None:
        return ""
    root = _ensure_tmp_dataset_root()

    # Reset the temp dir each time for clarity
    try:
        if root.exists():
            shutil.rmtree(root)
        root.mkdir(parents=True, exist_ok=True)
    except Exception:
        # As a fallback, create a fresh temp dir
        root = Path(tempfile.mkdtemp(prefix="ve2plume_dataset_"))
        st.session_state["_ve2plume_tmp_dataset_root"] = str(root)

    buf = io.BytesIO(uploaded_zip.getvalue())
    with zipfile.ZipFile(buf) as zf:
        zf.extractall(root)

    # Find LMDB data file; accept both 'data.mdb' and (rarely) 'data' if extensions are stripped.
    cand = next(root.rglob("data.mdb"), None)
    if cand is None:
        cand = next(root.rglob("data"), None)
    return str(cand.parent) if cand is not None else str(root)
...
ds_mode = st.radio(
    "Dataset source",
    ["Local folder", "Upload ZIP"],
    horizontal=True,
    help="Use 'Upload ZIP' on Streamlit Cloud or when the app cannot access your local filesystem.",
)

if ds_mode == "Upload ZIP":
    uploaded_zip = st.file_uploader(
        "Upload dataset ZIP (must contain data.mdb and optionally lock.mdb)",
        type=["zip"],
        help="ZIP the entire dataset folder (containing data.mdb/lock.mdb) and upload it here.",
    )
    dataset_dir = _prepare_uploaded_dataset_from_zip(uploaded_zip) if uploaded_zip is not None else ""
else:
    dataset_dir = st.text_input(
        "Dataset folder (contains data.mdb/lock.mdb)",
        value=default_dataset,
        help="Example: C:\Users\hungv\Downloads\ve2plume_streamlit_tool\dataset",
    )
    if dataset_dir.strip() == "":
        dataset_dir = default_dataset

# Normalize path (also supports pasting the data.mdb file path)
p_ds = Path(dataset_dir).expanduser()
if p_ds.is_file():
    p_ds = p_ds.parent
try:
    p_ds = p_ds.resolve()
    ...
    p_ds = p_ds.absolute()
dataset_dir = str(p_ds)

st.caption(f"Resolved: {dataset_dir}")
...
if str(dataset_dir).strip() == "":
    st.warning("No dataset selected. Choose a local folder or upload a ZIP in the sidebar.")
    st.stop()

# If the user pasted a file path (e.g., ...\\dataset\\data.mdb), accept its parent folder
if p_dataset.is_file():
    p_dataset = p_dataset.parent
    dataset_dir = str(p_dataset)

if not p_dataset.exists():
    st.warning(
        "Dataset path not found by the app. If you are running remotely (e.g., Streamlit Cloud), "
        "use **Upload ZIP** in the sidebar. If you are running locally, paste the full folder path."
    )
    st.info(r"Windows example: C:\Users\<you>\Downloads\ve2plume_streamlit_tool\dataset")
    st.stop()

# Must contain LMDB data file
has_data = (p_dataset / "data.mdb").exists() or (p_dataset / "data").exists()
if not has_data:
    st.warning(
        "Folder found, but LMDB file **data.mdb** was not detected. "
        "Make sure this folder contains data.mdb (and optionally lock.mdb)."
    )
    # Quick diagnostics
    try:
        entries = [p.name for p in sorted(p_dataset.iterdir())][:50]
